...
<div class='row'><div class='col s12'>
      <ul class='tabs'>
        <li class='tab col s4'><a class='active' href='#technews'>Tech News</a></li>
        <li class='tab col s6'><a href='#currentaffairs'>Current Affairs</a></li>   
      </ul>
    </div>");
echo("<div id='technews' class='col s12'>");
$page=$_GET['page'];
$sql1="select * from TBL_NAME where Category='TechNews'";
$result1=mysqli_query($con,$sql1);
$count=mysqli_num_rows($result1);
if($count!=0)
{
$total=ceil($count/3);
$sql="select * from TBL_NAME where Category='TechNews' ORDER BY Id DESC LIMIT $start_from,3";
...
<div class='col s4'>
    <div class='card small'>
            ...
            <a style='float:right;' class='dropdown-button btn' href='#' data-activates='dropdown-technews$Id'>Share</a>

  <!-- Dropdown Structure -->
  <ul id='dropdown-technews$Id' class='dropdown-content'>
...
for ($i=1; $i<=$total; $i++) { 
            echo "<li class='waves-effect'><a href='news.php?page=".$i."'>".$i."</a><li>"; 
};
echo("</ul></div></div>");
}
else
{
  echo("<h1>Nothing to display right now.</h1></div></div>");
}
echo("<div class='row'>");
echo("<div id='currentaffairs' class='col s12'>");
$page=$_GET['page'];
$sql1="select * from TBL_NAME where Category='CurrentAffairs'";
$result1=mysqli_query($con,$sql1);
$count=mysqli_num_rows($result1);
if($count!=0)
{
$total=ceil($count/3);
$sql="select * from TBL_NAME where Category='CurrentAffairs' ORDER BY Id DESC LIMIT $start_from,3";
$result=mysqli_query($con,$sql);
while($row=mysqli_fetch_array($result,MYSQL_ASSOC))
{
 $title=$row['Title'];
...
echo("<ul class='pagination'>");
for ($i=1; $i<=$total; $i++) { 
            echo "<li class='waves-effect'><a href='news.php#currentaffairs?page=".$i."'>".$i."</a><li>"; 
};
echo("</ul></div></div>");
}
else
